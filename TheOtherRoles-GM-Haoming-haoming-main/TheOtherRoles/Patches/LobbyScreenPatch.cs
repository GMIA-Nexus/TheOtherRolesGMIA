            ...
            if (LobbyText)
            {
                ...
                if (GameId != 0 && GameId != 32)
                {
                    ...
                    if (code != "")
                    {
                        ...
                        LobbyText.GetComponent<TMPro.TextMeshPro>().text = string.Format(ModTranslation.getString("lobbyJoinBindPrev"), code);

                    }
                }
                if (code2 != "")
                {
                    LobbyText.GetComponent<TMPro.TextMeshPro>().text += string.Format(ModTranslation.getString("lobbyJoinBindnClipboard"), code2Disp);
                }
            }
        }
    }
